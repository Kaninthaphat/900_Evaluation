...
public class view_910_waste_check {
	...
	{		
		...
		if(!scan_qr.equals(""))
		{
	        ...
	        String StrSQL = "EXEC [913_WASTE_CHECK] @START_DATE = '"+scan_qr+"'";	
    ...
    public void exportListboxToExcel() throws Exception {
        ...
        AMedia amedia = new AMedia("waste_control.xls", "xls", "application/file", out.toByteArray());
		    	...
		    	for (Iterator<model_scrap_inform> k = model_list_scrap.iterator(); k.hasNext();) {
		    		model_scrap_inform i_count = k.next();	
		   			if(((ListModelList<model_scrap_inform>)model_list_scrap).isSelected(i_count)){
						     ...
						     java.sql.CallableStatement cstmt = conn.prepareCall("{call [914_WASTE_CONFIRM](?,?,?,?)}");
