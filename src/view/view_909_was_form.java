...
public class view_909_was_form {
	...
	public static void setPath_name(String path_name) {
		view_909_was_form.path_name = path_name;
	}
	...
	{		
        ...
        String StrSQL = "EXEC [GET_B_WASTE_GROUP] ";
	...
	{		
        ...
        List<model_comboitem> model_list = new ArrayList<>();
        SqlSelection sqlsel = new SqlSelection();
        String StrSQL = "SELECT id,row_number,mattype_name,mattype_desc FROM dbo.B_WASTE_MATTYPE ORDER BY row_number";	
	...
	{		
        ...
        List<model_comboitem> model_list = new ArrayList<>();
        SqlSelection sqlsel = new SqlSelection();
        String StrSQL = "SELECT id,row_number,scraptype_name,scraptype_desc FROM dbo.B_WASTE_TYPE ORDER BY row_number";	
	...
	{		
		...
		try{			                 
			...
			if(id == 0){
			    ...
			    if(factory_id != 0 && mat_type_id != 0 && sc_type_id != 0 && pd_group_id != 0){				    					
			    	
			    	
			    	java.sql.CallableStatement cstmt = conn.prepareCall("{call [910_WASTEFORM_INS](?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
					...
					Clients.showNotification("WARNING_MESSAGE.!  <br/> *Factory, *Mat Type, *Scrap Type,*Productgroup,*Weight <br/>",
				    Clients.NOTIFICATION_TYPE_WARNING, null, "middle_center", 3100);
				}
	...
	{				
	        ...
	        SqlSelection sqlsel = new SqlSelection();
	        String StrSQL = "EXEC [911_WASTE_FORM_Q] @START_DATE = '"+startDate_string+"',"
	...
	{						
		defulat_book("SUST-FM-EMS-11.xls"); 
		Connection conn = objConnection.getConnection();
   		conn = objConnection.getConnection();
		try {    
	        		   ...
	        		   if(item_h.getStatus_id() == 1 || item_h.getStatus_id() == 2){
	        			 ...
	        			 if(sum_row == 1 || sum_row == 2){
	        			   ...
	        			   if(i_row % 2 == 0){
	        				...
	        				{
		        					java.sql.CallableStatement cstmt = conn.prepareCall("{call [912_WASTEFORM_CANCEL](?,?,?)}");			               		
			                    ...
			                    Range range41 = Ranges.range(Spreadsheet.getSelectedSheet(),row_15,1);
			                    range41.setCellEditText(i_row+"");
			                    			                    			                    			                    
			                    Range range4 = Ranges.range(Spreadsheet.getSelectedSheet(),row_15,2);
			                    ...
			                    range13.setCellEditText("*** หมายเหตุ จำกัดน้ำหนักต่อถุงไม่เกิน 20 Kgs.");
	...
	private void set_qrcode(String code,int row,int column) throws WriterException, IOException{
        ...
        try{
            SheetOperationUtil.addPicture(selection,new AImage(WebApps.getCurrent().getResource("/qrcode/waste/"+code+".png")));
        }catch(IOException e){
		    	...
		    	for (Iterator<model_scrap_inform> k = model_list_scrap.iterator(); k.hasNext();) {
		    		model_scrap_inform i_count = k.next();	
		   			if(((ListModelList<model_scrap_inform>)model_list_scrap).isSelected(i_count)){
		   				 ...
		   				 System.out.println("row="+i_count.getId());
		   			}
				}
